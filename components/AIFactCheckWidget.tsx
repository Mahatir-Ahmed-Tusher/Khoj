...
এই রিপোর্টটি Khoj ফ্যাক্ট চেকার দ্বারা তৈরি করা হয়েছে।
  ...
  return (
    <>
      {/* Desktop Widget */}
      <div
        className="hidden lg:block bg-white rounded-xl p-4 sticky top-4 border border-gray-300"
        style={{
        ...
        {aiFactChecks.length === 0 ? (
          ...
          <div className="space-y-3">
            {aiFactChecks.slice(0, 3).map((check) => (
              <div
                key={check.id}
                className="bg-gray-50 hover:bg-gray-100 rounded-lg p-3 transition-all duration-300 border border-gray-200 hover:border-gray-300 max-w-full"
              >
                <div className="flex items-center space-x-3 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between mb-1">
                      <span
                        className={`inline-block px-1.5 py-0.5 rounded-full text-xs font-medium ${getVerdictColor(check.verdict)}`}
                      >
                    ...
                    <h4
                      className="font-medium font-tiro-bangla text-gray-800 text-sm cursor-pointer hover:text-primary-600 transition-colors overflow-hidden"
                      onClick={() =>
                        (window.location.href = `/factcheck-view/${check.id}`)
                      }
                      title={check.query}
                    >
                      <span className="block truncate max-w-full">
                        {check.query.length > 45
                          ? `${check.query.substring(0, 45)}...`
                          : check.query}
                      </span>
                    </h4>
      ...
      <div
        className={`lg:hidden fixed top-0 left-0 h-full w-80 bg-white shadow-xl transform transition-transform duration-300 ease-in-out z-40 ${
              ...
              <div className="space-y-3">
                {aiFactChecks.map((check) => (
                  ...
                  >
                    ...
                    <p
                      className="text-sm text-gray-900 font-tiro-bangla mb-1 cursor-pointer hover:text-primary-600 transition-colors overflow-hidden"
                      onClick={() => {
                        setIsOpen(false);
                        window.location.href = `/factcheck-view/${check.id}`;
                      }}
