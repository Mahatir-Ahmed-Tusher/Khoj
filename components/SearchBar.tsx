    ...
    const handleVoiceResult = (event: CustomEvent) => {
      ...
      if (transcript.trim()) {
        if (onSearch) {
          ...
          router.push(`/factcheck-detail?query=${encodeURIComponent(transcript.trim())}`)
        }
      }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNewsCheckMode && query.trim()) {
      ...
      if (urlPattern.test(query.trim())) {
        router.push(`/news-verification?url=${encodeURIComponent(query.trim())}`)
        return
      } else {
    ...
    <form onSubmit={handleSubmit} className={`relative ${className}`} data-tour={dataTour}>
      <div className="relative">
        ...
        <button
          ...
          title={isAIImageCheckMode ? "Upload Image for AI Detection" : "AI ছবি যাচাই বাটনে ক্লিক করুন"}
        ...
        >
          <Image
            ...
            height={16}
            className={`w-4 h-4 md:w-5 md:h-5 transition-all duration-200 ${
              isImageSearchMode ? 'filter-none' : 'filter grayscale hover:grayscale-0'
            }`}
          />
        </button>
