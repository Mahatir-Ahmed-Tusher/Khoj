    ...
    if (type === "fact-check") {
      ...
      try {
        ...
        if (apiKey) {
          try {
            console.log(
              "🤖 Trying Gemini (gemini-2.5-flash) for fact-check..."
            );
...
4. **Verdict**: Provide a clear verdict (True/False/unverified/Unverified)
...
Please provide a comprehensive fact-check report in Bengali following the structure above.`;
          ...
          console.log(
            "⚠️ GEMINI_API_KEY_2 not configured, using Groq for fact-check..."
          );
        }
...
Please provide a comprehensive fact-check report in Bengali following the structure above.`;
      ...
      try {
        ...
        if (apiKey) {
          try {
            console.log(
              "🤖 Trying Gemini (gemini-2.5-flash) for general chat..."
            );
            const genAI = new GoogleGenerativeAI(apiKey);
            const model = genAI.getGenerativeModel({
              model: "gemini-2.5-flash",
...
Please provide a detailed and well-formatted answer in Bengali.`;
          ...
          console.log(
            "⚠️ GEMINI_API_KEY_2 not configured, using Groq for general chat..."
          );
        }
