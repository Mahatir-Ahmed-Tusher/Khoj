...
async function classifyGeography(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `Geography classification API failed: ${response.status} - ${errorText}`
      );
      throw new Error(
        `Geography classification API failed: ${response.status}`
      );
    }
  ...
  for (const tier of tiers) {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `✅ Tier ${tier.tier} provided sufficient results, may stop or continue based on total count`
        );
    ...
    if (remainingSlots > 0) {
      ...
      if (generalResults.results && generalResults.results.length > 0) {
        ...
        if (socialMediaGeneral.length > 0) {
          ...
          socialMediaSources.push(...socialMediaWithMetadata);
          console.log(
            `📱 General search found ${socialMediaGeneral.length} social media sources (not counted as verified sources)`
          );
        }
        ...
        if (generalWithTier.length > 0) {
          ...
          console.log(
            `✅ General search found ${generalWithTier.length} additional relevant sources outside tiers (total: ${allResults.length})`
          );
        }
      }
    }
    ...
    try {
      ...
      if (extendedResults.results && extendedResults.results.length > 0) {
        ...
        if (socialMediaExtended.length > 0) {
          ...
          socialMediaSources.push(...socialMediaWithMetadata);
          console.log(
            `📱 Extended search found ${socialMediaExtended.length} social media sources (not counted as verified sources)`
          );
        }
        ...
        console.log(
          `✅ Extended search found ${filteredExtended.length} additional sources (total: ${allResults.length})`
        );
      }
    } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (relevantResults.length < 10) {
      ...
      console.log(
        `✅ Found ${relevantResults.length} results with relaxed threshold`
      );
    }
