...
interface PlagiarismResult {
  success: boolean
  type: 'plagiarism'
  verdict: 'plagiarized' | 'original' | 'unverified'
...
export default function PlagTestPage() {
  const [text, setText] = useState('')
  const [isLoading, setIsLoading] = useState(false)
  const [result, setResult] = useState<PlagiarismResult | null>(null)
  const [error, setError] = useState('')

  // Get text from URL parameters or sessionStorage
  useEffect(() => {
    ...
    if (textFromUrl) {
      ...
      // Auto-start plagiarism check
      setTimeout(() => {
        handlePlagiarismCheck(decodeURIComponent(textFromUrl))
      }, 500)
    } else {
      // Check sessionStorage for text
      const storedText = sessionStorage.getItem('plagiarismText')
      if (storedText) {
        setText(storedText)
        sessionStorage.removeItem('plagiarismText')
        // Auto-start plagiarism check
        setTimeout(() => {
          handlePlagiarismCheck(storedText)
        }, 500)
      }
    }
  }, [])

  const handlePlagiarismCheck = async (textToCheck?: string) => {
    const textToAnalyze = textToCheck || text.trim()
    ...
    if (textToAnalyze.length < 100) {
      setError('সর্বনিম্ন ১০০ অক্ষর প্রয়োজন (প্লেজিয়ারিজম সনাক্তকরণের জন্য)')
      return
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 font-tiro-bangla">
            লেখা চুরি করা হয়েছে কিনা তা সনাক্ত করুন
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              className="w-full bg-red-600 text-white py-3 px-4 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed font-tiro-bangla"
            >
              {isLoading ? 'যাচাই হচ্ছে...' : 'প্লেজিয়ারিজম যাচাই করুন'}
            </button>
          </form>
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
          ...
          <p className="text-gray-600 mb-6 font-tiro-bangla">
            আমাদের প্লেজিয়ারিজম ডিটেকশন সিস্টেম লেখার বিভিন্ন বৈশিষ্ট্য বিশ্লেষণ করে চুরি করা কিনা তা সনাক্ত করে।
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center">
              ...
              <h3 className="font-semibold text-gray-900 mb-2 font-tiro-bangla">প্লেজিয়ারিজম বিশ্লেষণ</h3>
